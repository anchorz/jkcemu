...
public class Z1013 extends EmuSys implements FDC8272.DriveSelector, Z80AddressListener, Z80PCListener {
	...
	public static final String PROP_ROMBASIC_ENABLED = PROP_ROMBASIC_PREFIX + "enabled";
	...
	public static final String[] basicTokens = { "END", "FOR", "NEXT", "DATA", // 0x80
			"INPUT", "DIM", "READ", "LET", "GOTO", "RUN", "IF", "RESTORE", "GOSUB", "RETURN", "REM", "STOP", "OUT",
			"ON", "NULL", "WAIT", // 0x90
			"DEF", "POKE", "DOKE", "AUTO", "LINES", "CLS", "WIDTH", "BYE", "!", "CALL", "PRINT", "CONT", "LIST",
			"CLEAR", "CLOAD", "CSAVE", // 0xA0
			"NEW", "TAB(", "TO", "FN", "SPC(", "THEN", "NOT", "STEP", "+", "-", "*", "/", "^", "AND", "OR", ">", // 0xB0
			"=", "<", "SGN", "INT", "ABS", "USR", "FRE", "INP", "POS", "SQR", "RND", "LN", "EXP", "COS", "SIN", "TAN", // 0xC0
			"ATN", "PEEK", "DEEK", "PI", "LEN", "STR$", "VAL", "ASC", "CHR$", "LEFT$", "RIGHT$", "MID$", "LOAD", "TRON",
			"TROFF", "EDIT", // 0xD0
			"ELSE", "INKEY$", "JOYST", "STRING$", "INSTR", "RENUMBER", "DELETE", "PAUSE", "BEEP", "WINDOW", "BORDER",
			"INK", "PAPER", "AT", "HSAVE", "HLOAD", // 0xE0
			"PSET", "PRES" };

	private static final String[] sysCallNames = { "OUTCH", "INCH", "PRST7", "INHEX", "INKEY", "INLIN", "OUTHX",
			"OUTHL", "CSAVE", "CLOAD", "MEM", "WIND", "OTHLS", "OUTDP", "OUTSP", "TRANS", "INSTR", "KILL", "HEXUM",
			"ALPHA" };

	/*
	 * Mapping der Zeichen ab Code 14 (Schachfiguren)
	 *
	 * Da die Z1013-Schachfiguren aus zwei uebereinander stehenden Zeichen bestehen,
	 * im Unicode aber es nur jeweils ein Zeichen dafuer gibt, wird das obere
	 * Zeichen in das Unicode-Zeichen gemappt und das untere in ein Leerzeichen.
	 */
	private static final int[] char14ToUnicode = { '\u265F', '\u265C', '\u265E', '\u0020', '\u265D', '\u0020', '\u265B',
			'\u265A', '\u0020', '\u2659', '\u2656', '\u2658', '\u0020', '\u2657', '\u0020', '\u2655', '\u2654',
			'\u0020' };

	private static final FloppyDiskInfo cpm64x16FloppyDisk = new FloppyDiskInfo("/disks/z1013/z1013cpm64x16.dump.gz",
			"Z1013 CP/M Boot-Diskette (64x16 Zeichen)", 2, 2048, true);

	private static final FloppyDiskInfo cpm80x25FloppyDisk = new FloppyDiskInfo("/disks/z1013/z1013cpm80x25.dump.gz",
			"Z1013 CP/M Boot-Diskette (80x25 Zeichen)", 2, 2048, true);

	private static final FloppyDiskInfo[] availableFloppyDisks = { cpm64x16FloppyDisk, cpm80x25FloppyDisk };

	private enum BasicType {
		Z1013_TINY, KC_RAM, KC_ROM
	};

	private enum UserPort {
		NONE, JOY_JUTE_6_1987, JOY_PRACTIC_4_1987, JOY_PRACTIC_1_1988, CENTR7_PRACTIC_2_1989, CENTR8_FA_10_1990
	};
	...
	public Z1013(EmuThread emuThread, Properties props) {
		...
		this.ramFloppy1 = RAMFloppy.prepare(this.emuThread.getRAMFloppy1(), "Z1013", RAMFloppy.RFType.MP_3_1988,
				"RAM-Floppy an E/A-Adressen 98h-9Fh", props, PROP_PREFIX + PROP_RF1_PREFIX);

		this.ramFloppy2 = RAMFloppy.prepare(this.emuThread.getRAMFloppy2(), "Z1013", RAMFloppy.RFType.MP_3_1988,
				"RAM-Floppy an E/A-Adressen 58h-5Fh", props, PROP_PREFIX + PROP_RF2_PREFIX);
		...
		if (emulatesGraphCCJ(props)) {
			this.graphCCJ = new GraphicCCJ(this.screenFrm,
					EmuUtil.getProperty(props, this.propPrefix + PROP_GCCJ_FONT_FILE));
		} else {
			this.graphCCJ = null;
		}
	...
	public static int getDefaultSpeedKHz(Properties props) {
		return EmuUtil.getProperty(props, EmuThread.PROP_SYSNAME).startsWith(SYSNAME_Z1013_01) ? 1000 : 2000;
	}

	public static String getTinyBasicProgram(EmuMemView memory) {
		return SourceUtil.getTinyBasicProgram(memory, 0x1152, memory.getMemWord(0x101F));
	}
    ...
    HashSet <String> callingConvention = new HashSet <String> ();
    HashMap <String,HashSet <String>> callingConventionTag = new HashMap <String,HashSet <String>> ();

    String mapRst20(int index) {
        String [] map= {"OUTCH","INCH","PRST7","INHEX",
    ...
    String mapSpv(int adr) {
        int index=(25*3)-(adr-0xffb2);
        String [] map= {"INKEY","POLL","STAT","SARUF",
                        "LORUF","ZMINI","DRDEL","DRAAK",
                        "BSDR","HARDC","DRZEL","BEEP",
                        "ASTA","BSTA","AIN","BIN",
                        "DRINI","ZEIDR","BLMK","BSMK",
                        "SUCHK","AKP","GETST","SOUND",
                        "BSAVE","BLOAD"};
        try {
            return map[index/3];
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    void formatCallingConvention() {
        for(String t:callingConvention) {
            HashSet <String> tag=callingConventionTag.get(t);
            String ids;            
            if (tag==null) {
               System.out.printf("<syscall type=\"%s\"/>\n",t);             
            } else {
               System.out.printf("<syscall type=\"%s\" tag=\"%s\"/>\n",t,tag.toString());
            }
        }
    }
        ...
        if ((origin<0xf000 && pc>=0xe000) || (origin<0xf000 && pc==0x0020)) {
            ...
            if (!newAdrItem.contains(id)) {
                ...
                if (pc==0xf000) {
                    //reset vector     
                    text="reset";       
                    //System.out.printf("PC: %04x -> %04x %08x %s\n",origin,pc,id,text);
                } else if (pc>=0xf000 && origin==0x0038) {
                    //rst38 vector     
                    text="monitor";       
                    //System.out.printf("PC: %04x -> %04x %08x %s\n",origin,pc,id,text);
                } else if (pc>=0xf000 && origin==0x0020) {
                    //rst20 vector     
                    text="rst20";       
                    //System.out.printf("PC: %04x -> %04x %08x %s\n",origin,pc,id,text);
                } else if (monitor202adr.get(pc)!=null) {    
                    text=String.format("202 %s",monitor202adr.get(pc));
                    HashSet <String> tag=callingConventionTag.get("202");
                    if (tag==null) {
                        tag=new HashSet <String> (); 
                        callingConventionTag.put("202",tag);            
                    }
                    tag.add(monitor202adr.get(pc));
                    callingConvention.add("202");       
                } else if (monitorA2adr.get(pc)!=null) {  
                    text=String.format("A.2 %s",monitorA2adr.get(pc));
                    HashSet <String> tag=callingConventionTag.get("A.2");
                    if (tag==null) {
                        tag=new HashSet <String> (); 
                        callingConventionTag.put("A.2",tag);            
                    }
                    tag.add(monitorA2adr.get(pc));
                    callingConvention.add("A.2");       
                } else if (pc>=0xffb2) {
                    //rst20 vector     
                    text=String.format("spv %s",mapSpv(pc));
                    //System.out.printf("PC: %04x -> %04x %08x %s\n",origin,pc,id,text);
                    HashSet <String> tag=callingConventionTag.get("spv");
                    if (tag==null) {
                        tag=new HashSet <String> (); 
                        callingConventionTag.put("spv",tag);            
                    }
                    tag.add(mapSpv(pc));
                    callingConvention.add("spv"); 
                } else if (pc==0x0020) {
                    int adr=this.getEmuThread().getZ80CPU().getRegSP();
                    adr=this.getEmuThread().getMemWord(adr);
                    adr=this.getEmuThread().getMemByte(adr,false);
                    text=String.format("%02x %s",adr,mapRst20(adr));
                    HashSet <String> tag=callingConventionTag.get("rst20");
                    if (tag==null) {
                        tag=new HashSet <String> ();                  
                        callingConventionTag.put("rst20",tag);            
                    }
	...
	public void appendStatusHTMLTo(StringBuilder buf, Z80CPU cpu) {
		...
		if (rom != null) {
			if (rom.length > 0) {
				buf.append(String.format(" (F000h-F%03Xh):", Math.min(rom.length, 0x1000) - 1));
	...
	public boolean canApplySettings(Properties props) {
		boolean rv = EmuUtil.getProperty(props, EmuThread.PROP_SYSNAME).equals(this.sysName);
		if (rv) {
			rv = TextUtil.equals(this.osFile, EmuUtil.getProperty(props, this.propPrefix + PROP_OS_FILE));
		}
		if (rv) {
			rv = TextUtil.equals(this.monCode, EmuUtil.getProperty(props, this.propPrefix + PROP_MONITOR));
		}
		if (rv && (getRAMEndAddr(props) != this.ramEndAddr)) {
			rv = false;
		}
		if (rv && emulatesModuleBasic(props) == (this.romBasic != null)) {
			if ((this.romBasic != null) && !TextUtil.equals(this.romBasicFile,
					EmuUtil.getProperty(props, this.propPrefix + PROP_ROMBASIC_PREFIX + PROP_FILE))) {
				rv = false;
			}
		} else {
			rv = false;
		...
		if (rv && emulatesModuleMegaROM(props) == (this.romMega != null)) {
			if ((this.romMega != null) && !TextUtil.equals(this.romMegaFile,
					EmuUtil.getProperty(props, this.propPrefix + PROP_ROMMEGA_PREFIX + PROP_FILE))) {
				rv = false;
			}
		} else {
			rv = false;
		...
		if (rv) {
			rv = RAMFloppy.complies(this.ramFloppy1, "Z1013", RAMFloppy.RFType.MP_3_1988, props,
					PROP_PREFIX + PROP_RF1_PREFIX);
		}
		if (rv) {
			rv = RAMFloppy.complies(this.ramFloppy2, "Z1013", RAMFloppy.RFType.MP_3_1988, props,
	...
	@Override
	public boolean keyPressed(int keyCode, boolean ctrlDown, boolean shiftDown) {
	...
	public void openBasicProgram() {
		...
		switch (OptionDlg.showOptionDlg(this.screenFrm,
				"W\u00E4hlen Sie bitte den BASIC-Interpreter aus,\n"
						+ "dessen BASIC-Programm ge\u00F6ffnet werden soll.",
				"BASIC-Interpreter", preIdx, "Z1013-Tiny-BASIC", "KC-BASIC (RAM-Version)", "KC-BASIC (ROM-Version)")) {
		case 0:
		...
		case 1:
			...
			text = SourceUtil.getBasicProgram(this.emuThread, 0x2C01, basicTokens);
			break;

		case 2:
			bType = BasicType.KC_ROM;
			text = SourceUtil.getBasicProgram(this.emuThread, 0x0401, basicTokens);
			break;
	...
	HashSet<Integer> newItem = new HashSet<Integer>();
    int used[]=new int[256];

	HashSet<Integer> newAdrItem = new HashSet<Integer>();

	static final int IOWrite = 0;
	static final int IORead = 1;

    String mapUsedIo(int io) {
        switch(io) {
            case 1: return "W";
            case 2: return "R";
            case 3: return "R/W";
        ...
        return String.format("%x",used[io]);
    }
    ...
    void formatIo() {
            for (int i:newItem) {
                int io=(i>>24) &0xff;
                used[io] |= 1<<(i&3);
                //System.out.printf("used P[%02x] = %x\n",io,used[io]);
            }
            for (int io=0; io<256;io++) {
                if (used[io]!=0) {
                    switch(io) {
                        case 0x00: 
                            System.out.printf("<port range=\"00\" mode=\"%s\" device=\"Userport: Tonausgabe, S3004, Joystick JU+TE PA0188, unbekannte Funktion\"/>\n",mapUsedIo(used[io]));
                            break;                  
                        case 0x01:
                        ...
                        case 0x03: 
                            ...
                            break;                  
                        case 0x04: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"[7]64/16[6]4Mhz[5]ZGBank[4]ROMdis[3]FZG, unbekannte Funktion\"/>\n",io,mapUsedIo(used[io]));
                            break;                  
                        case 0x08: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"Spaltentreiber: Tastatur+RB/8x4, unbekannte Funktion\"/>\n",io,mapUsedIo(used[io]));
                            break;
                        case 0x10: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"TTL-Tester\"/>\n",io,mapUsedIo(used[io]));
                            break;
                        case 0x11: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"TTL-Tester\"/>\n",io,mapUsedIo(used[io]));
                            break;
                        ...
                        case 0x13: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"TTL-Tester\"/>\n",io,mapUsedIo(used[io]));
                            break;
                        //#Syntax Gerätetreiber: Funktion
                        case 0x50: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"Centronics: Data\"/>\n",io,mapUsedIo(used[io]));
                            break;
                        case 0x51: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"Centronics: Initalisierung\"/>\n",io,mapUsedIo(used[io]));
                            break;
                        case 0x52: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"Centronics: Status, Floppy 1541: IO\"/>\n",io,mapUsedIo(used[io])); //Bit 0
                            break;
                        case 0x53: 
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"Centronics: Initalisierung, Floppy 1541: Initalisierung\"/>\n",io,mapUsedIo(used[io])); //Bit 0 busy, bit 543 O-1541 Data/Clk/at-OUT, bit 67 Data/Clk-In
                            break;
                        default:
                            System.out.printf("<port range=\"%02x\" mode=\"%s\" device=\"unbekannte Funktion\"/>\n",io,mapUsedIo(used[io]));
                            break;
                    }
                }
            }
	...
	void recordIO(int type, int port) {
		int pc = this.getEmuThread().getZ80CPU().getRegPC() & 0xffff;
		int id = (port & 0xff) << 24;
		id |= pc<<8;
		id |= type;

		if (!newItem.contains(id) && pc<0xf000) {
   			newItem.add(id);
			System.out.printf("IO: %02x %04x %s %08x\n", port & 0xFF, pc, type==IOWrite?"Wr":"Rd",id);
            formatIo();
            formatCallingConvention();
		}
	}
	...
	public int readIOByte(int port, int tStates) {
		...
		if ((this.gide != null) && (((port & 0xF0) == 0x40) || ((port & 0xF0) == 0x80))) {
	...
	@Override
	public int reassembleSysCall(Z80MemView memory, int addr, StringBuilder buf, boolean sourceOnly, int colMnemonic,
			int colArgs, int colRemark) {
		int rv = 0;
		int bol = buf.length();
		...
		if (rv > 0) {
			...
			rv++;
			if (b == 2) {
				rv += reassStringBit7(memory, addr + 1, buf, sourceOnly, colMnemonic, colArgs);
			}
		}
		return rv;
	}
	...
	public void reset(EmuThread.ResetLevel resetLevel, Properties props) {
        ...
        newItem = new HashSet<Integer>();
        newAdrItem = new HashSet<Integer>();
        callingConvention  = new HashSet<String>();
        callingConventionTag  = new HashMap<String,HashSet<String>>();
        used=new int[256];
	...
	public void saveBasicProgram() {
		...
		case 0:
			...
			if ((endAddr > 0x1152) && (this.emuThread.getMemByte(endAddr - 1, false) == 0x0D)) {
		...
		if (!cancelled) {
			if ((begAddr > 0) && (endAddr > begAddr)) {
				...
				(new SaveDlg(this.screenFrm, begAddr, endAddr, title, dstBasicType, null)).setVisible(true);
			} else {
				showNoBasic();
			}
		}
	...
	public boolean setMemByte(int addr, int value) {
		...
		boolean done = false;
		if ((addr >= 0xC000) && (addr < 0xEC00)) {
			if ((this.romBasic != null) || ((this.romMega != null) && (addr < 0xE800))) {
				done = false;
			}
		...
		} else if ((addr >= 0xF000) && !this.romDisabled && !v128Settings.bwsDisabled && (this.osBytes != null)) {
	...
	public boolean supportsKeyboardFld() {
		...
		KeyboardMatrix kbMatrix = this.keyboard.getKeyboardMatrix();
		if (kbMatrix != null) {
			if ((kbMatrix instanceof KeyboardMatrix8x4) || (kbMatrix instanceof KeyboardMatrix8x8)) {
				rv = true;
			}
		}
		return rv;
	...
	public boolean supportsPrinter() {
		return this.catchPrintCalls || (this.userPort == UserPort.CENTR7_PRACTIC_2_1989)
	...
	@Override
	public void updSysCells(int begAddr, int len, FileFormat fileFmt, int fileType) {
		SourceUtil.updKCBasicSysCells(this.emuThread, begAddr, len, fileFmt, fileType);
	}

	@Override
	public void writeIOByte(int port, int value, int tStates) {
		recordIO(IOWrite, port);
		// port &= 0xFF;
		if ((port & 0xfc) == 4) {
			if (sysName.equals(SYSNAME_Z1013_128)) {
				...
				if ((this.altFontEnabled != oldAltFontEnabled) || (this.mode64x16 != oldMode64x16)) {
					this.screenFrm.setScreenDirty(true);
				}
				...
				case 0:
					...
					if (this.userPort == UserPort.JOY_PRACTIC_1_1988) {
						...
						if (this.pio.getModePortA() == Z80PIO.Mode.BYTE_OUT) {
							...
							/*
							 * Drucker soll bei 2 MHz nach 1/100 Sekunde Bereitschaft melden
							 */
	...
	public void writeMemByte(int addr, int value) {
		...
		/*
		 * Wenn die zusaetztliche Grafikkarte aktiv ist und der Adressbereich EC00-EFFF
		 * der Reihe nach beschrieben wird, wird davon ausgegangen, dass wieder die
		 * Standardbildschirmausgabe aktiv sein soll. In dem Fall wird deshalb wieder
		 * zurueckgeschaltet.
		 */
		if ((addr >= 0xEC00) && (addr < 0xF000) && this.graphCCJActive) {
			if ((addr == 0xEC00) || ((addr > 0xEC00) && (addr < 0xEFFF) && (addr == (this.lastWrittenAddr + 1)))) {
				this.lastWrittenAddr = addr;
			} else if ((addr == 0xEFFF) && (addr == (this.lastWrittenAddr + 1))) {
				this.graphCCJActive = false;
				this.screenFrm.fireScreenSizeChanged();
			} else {
				this.lastWrittenAddr = -1;
	...
	private void applyUserPortSettings(Properties props) {
		String text = EmuUtil.getProperty(props, this.propPrefix + PROP_USERPORT);
	...
	private synchronized void checkAddPCListener(Properties props) {
		this.pasteFast = EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_PASTE_FAST, true);
		...
		if (this.pasteFast) {
			String monText = EmuUtil.getProperty(props, this.propPrefix + PROP_MONITOR);
		...
		this.catchPrintCalls = EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_CATCH_PRINT_CALLS, true);
		...
		this.catchJoyCalls = EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_CATCH_JOY_CALLS, true);
		if (this.catchJoyCalls) {
			addrs.add(0xFFBB);
		}
		int[] a = null;
	...
	private boolean emulatesFloppyDisk(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_FDC_ENABLED, false);
	}

	private boolean emulatesGraphCCJ(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_GCCJ_ENABLED, false);
	}

	private boolean emulatesGraphic(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_GRAPHIC_ENABLED, false);
	}

	private boolean emulatesKCNet(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_KCNET_ENABLED, false);
	}

	private boolean emulatesModuleBasic(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_ROMBASIC_ENABLED, false);
	}

	private boolean emulatesModuleMegaROM(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_ROMMEGA_ENABLED, false);
	}

	private boolean emulatesRTC(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_RTC_ENABLED, false);
	}

	private boolean emulatesUSB(Properties props) {
		return EmuUtil.getBooleanProperty(props, this.propPrefix + PROP_VDIP_ENABLED, false);
	}
	...
	private void loadFonts(Properties props) {
		this.stdFontBytes = readFontByProperty(props, this.propPrefix + PROP_FONT_FILE, 0x1000);
		if (this.stdFontBytes != null) {
			if (this.stdFontBytes.length >= 0x1000) {
				this.altFontBytes = Arrays.copyOfRange(this.stdFontBytes, 0x0800, 0x1000);
			} else {
		...
		if (this.graphCCJ != null) {
			this.graphCCJ.loadFont(EmuUtil.getProperty(props, this.propPrefix + PROP_GCCJ_FONT_FILE));
		}
	}

	private void loadROMs(Properties props) {
		this.monCode = EmuUtil.getProperty(props, this.propPrefix + PROP_MONITOR);
		this.osFile = EmuUtil.getProperty(props, this.propPrefix + PROP_OS_FILE);
		...
		if (emulatesModuleBasic(props)) {
			this.romBasicFile = EmuUtil.getProperty(props, this.propPrefix + PROP_ROMBASIC_PREFIX + PROP_FILE);
			this.romBasic = readROMFile(this.romBasicFile, 0x2C00, "KC-BASIC-Modul");
			...
			this.romMegaFile = EmuUtil.getProperty(props, this.propPrefix + PROP_ROMMEGA_PREFIX + PROP_FILE);
			this.romMega = readROMFile(this.romMegaFile, 0x280000, "Mega-ROM-Modul");
		}
		loadFonts(props);
	}

	/*
	 * Joystick-Anschluss nach Ju+Te 6/1987 (alle L-aktiv): Bit 4: runter Bit 5:
	 * links Bit 6: rechts Bit 7: hoch
	 *
	 ...
	 * angegeben, die bei nur einem Joystick gelesen werden. Demnach ist Bit 0 immer
	 * 1 (als Grund wird ein angeschlossener Piezosummer genannt), und die Bits 1
	 * bis 3 sind immer 0. Um auch dieser Tabelle gerecht zu werden, wird hier nur
	 * ein Joystick emuliert und die Bits 0 bis 3 entsprechend den Werten der
	 * Tabelle gesetzt.
	 *
	 * Zu den Aktionsknoepfen steht nur, dass mehrere Leitungen gleichzeitig
	 * selektiert werden sollen, die mit dem Hebel nicht gleichzeitig ausgeloest
	 * werden koennen, also z.B. rechts+links, oben+unten oder alle vier Richtungen.
	 * Es wird deshalb hier adaequat zum Joystick-Anschluss nach practic 4/1987
	 * vorgegangen: Der Aktionsknopf waehlt alle vier Richtungen gleichzeitig aus.
	 */
